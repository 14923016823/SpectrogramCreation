...
f_stft : (nfft,) frequencies in Hz, ascending and fftshifted:
         -fs/2 .. +fs/2 - df.  Matches pass_strength's ascending-f convention.
Zxx    : (nfft, n_frames) complex STFT, indexed [frequency, frame] -- the exact
         layout pass_strength expects (it forms |Zxx|**2 and indexes [m, k]).
...
* scaling='density' makes |Zxx|**2 a PSD estimate (divides by fs * sum(w**2)).
  ...
  the choice of scaling shifts N0 but does not change SNR; pick what you like.
* The window centre is used as each frame's timestamp so the frame's energy is
  associated with the middle of its support, which is what aligns cleanly with a
  Doppler/carrier track sampled on its own time grid.
"""
...
def stft(x, nfft, hop, fs, window="hann", scaling="density", center=True):
    ...
    # Build frames (n_frames, nfft) as a strided view, keep every hop-th, then
    # window. The multiply materialises only the retained frames.
